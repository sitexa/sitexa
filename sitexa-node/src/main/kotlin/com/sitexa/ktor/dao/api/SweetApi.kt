...
interface SweetApi {
    ...
    @GET("/sweet-top/{count}/{page}")
    fun topSweet(@Path("count") count: Int, @Path("page") page: Int): Call<List<Int>>

    @GET("/sweet-latest/{count}/{page}")
    fun latestSweet(@Path("count") count: Int, @Path("page") page: Int): Call<List<Int>>

    @GET("/sweet-reply-count/{id}")
    fun countSweetReplies(@Path("id") id: Int): Call<Int>

    @GET("/sweet-replies/{id}")
    fun getReplies(@Path("id") id: Int): Call<List<Sweet>>

    @GET("/sweet-user/{user}")
    fun getUserSweets(@Path("user") user: String): Call<List<Int>>

    @FormUrlEncoded
    @POST("/sweet-new")
    fun createSweet(@Field("user") user: String, @Field("text") text: String, @Field("replyTo") replyTo: Int?): Call<ApiResult>

    @GET("/sweet-del")
    fun deleteSweet(@Query("id") id: Int): Call<ApiResult>

    @FormUrlEncoded
    @POST("/sweet-upd")
    fun updateSweet(@Field("id") id: Int, @Field("text") text: String): Call<ApiResult>

    @FormUrlEncoded
    @POST("/media-new")
    fun createMedia(@Field("refId") refId: Int, @Field("fileName") fileName: String, @Field("fileType") fileType: String?, @Field("title") title: String?, @Field("sortOrder") sortOrder: Int?): Call<ApiResult>

    @GET("/media-del")
    fun deleteMedia(@Query("id") id: Int): Call<ApiResult>

    @GET("/media/{name}/{type}")
    fun viewMedia(@Path("name") name: String, @Path("type") type: String): Call<ResponseBody>

    @GET("/media/{id}")
    fun getMedia(@Path("id") id: Int): Call<Media>

    @GET("/mediasBySweet/{refId}")
    fun getMediasBySweet(@Path("refId") refId: Int): Call<List<Int>>
